...
def upload_file_to_bronze(spark: SparkSession, logger: logging, entity: str, path_data: str, 
    ...
    # Resolve source path
    if ENTITY_LANDING.get(entity, False):
        path_data_entity = f"{path_data}/{entity}/landing"
    else:
        path_data_entity = f"{path_data}/{entity}"
    ...
    try:
        ...
        spark.read.table(f"{BRONZE_COMMON}.{entity}").show(truncate=False)
        logger.info("-" * 80)
